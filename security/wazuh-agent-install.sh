...
#        Versão: 2.1 - CORRIGIDA PARA PROXMOX 9.1                            #
#                                                                              #
...
║                                                                              ║
║        WAZUH AGENT 4.14.1 - AUTOMAÇÃO PROXMOX VE                           ║
║        Deployment com Validações e Tratamento de Erros (v2.1)              ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
...
validate_prerequisites() {
    ...
    # Detectar Proxmox (múltiplas formas de verificação)
    ...
    elif command -v pvecm &> /dev/null || command -v pveversion &> /dev/null; then
        log_success "Proxmox VE detectado via comando 'pve'"
    else
        log_warning "Proxmox VE não detectado explicitamente"
        log_warning "Se este é um Proxmox, execute: apt-get install proxmox-ve"
    fi
...
configure_wazuh_agent() {
    ...
    # IMPORTANTE: Remover tags inválidas antes de adicionar novas configurações
    log_info "Limpando configuração de tags inválidas..."
    sed -i '/<system_inventory>/,/<\/system_inventory>/d' "$AGENT_CONFIG"
    log_success "Tags inválidas removidas"
    
    # Configurar agente name e manager PRIMEIRO
    ...
    # Configurar Manager
    log_info "Configurando Manager: ${WAZUH_MANAGER}:${WAZUH_MANAGER_PORT}"
    sed -i "s/<address>.*<\/address>/<address>$WAZUH_MANAGER<\/address>/g" "$AGENT_CONFIG"
    sed -i "s/<port>.*<\/port>/<port>$WAZUH_MANAGER_PORT<\/port>/g" "$AGENT_CONFIG"
    
    # Injetar configuração para Proxmox CORRETAMENTE
    log_info "Injetando configuração para monitoramento Proxmox..."
    
    # Localizar a linha </ossec_config> para inserir ANTES dela
    if grep -q "</ossec_config>" "$AGENT_CONFIG"; then
        # Criar arquivo de configuração temporário SEM tags inválidas
  ...
  <!-- Monitoramento de Integridade de Arquivo PVE -->
  <syscheck>
    <frequency>43200</frequency>
    <directories realtime="yes" report_changes="yes">/etc/pve</directories>
...
PROXMOX_CONFIG
        ...
        # Inserir configuração ANTES de </ossec_config>
        sed -i '/<\/ossec_config>/e cat /tmp/proxmox_config.xml' "$AGENT_CONFIG"
        log_success "Configuração Proxmox injetada corretamente"
        rm -f /tmp/proxmox_config.xml
    else
        log_warning "Tag </ossec_config> não encontrada, usando configuração básica"
    fi
    ...
    if command -v xmllint &> /dev/null; then
        if xmllint --noout "$AGENT_CONFIG" 2>&1 | tee -a "$LOG_FILE"; then
            ...
            xmllint "$AGENT_CONFIG" 2>&1 | tee -a "$LOG_FILE" | head -20
        ...
        apt-get install -y libxml2 2>&1 | tail -3 >> "$LOG_FILE"
        
        if command -v xmllint &> /dev/null; then
            if xmllint --noout "$AGENT_CONFIG" 2>&1 | tee -a "$LOG_FILE"; then
                log_success "Sintaxe XML validada"
            else
                log_error "Erro na sintaxe XML detectado"
                cp "$BACKUP_FILE" "$AGENT_CONFIG"
                exit 1
            fi
        else
...
start_wazuh_agent() {
    ...
    if systemctl start wazuh-agent 2>&1 | tee -a "$LOG_FILE"; then
        ...
        systemctl status wazuh-agent -l 2>&1 | tee -a "$LOG_FILE"
        
        log_info "Tentando iniciar com wazuh-control..."
        /var/ossec/bin/wazuh-control start 2>&1 | tee -a "$LOG_FILE" || true
        
        log_info "Verificando arquivo de configuração..."
        if command -v xmllint &> /dev/null; then
            xmllint --noout "$AGENT_CONFIG" 2>&1 | tee -a "$LOG_FILE" || true
        fi
...
verify_installation() {
    ...
    # Verificar logs
    log_info "Últimas linhas do log do Wazuh:"
    if [ -f /var/ossec/logs/ossec.log ]; then
        tail -5 /var/ossec/logs/ossec.log 2>/dev/null | tee -a "$LOG_FILE" || true
    fi
}
...
Próximas Etapas:
  ...
  4. Registre o agente no Proxmox em: /etc/pve/nodes/<NODE>/wazuh-config
...
Comandos Úteis:
  ...
  • Validar XML: xmllint --noout /var/ossec/etc/ossec.conf

Troubleshooting:
  • Se houver erro de configuração: restaure o backup
    cp $BACKUP_FILE /var/ossec/etc/ossec.conf
